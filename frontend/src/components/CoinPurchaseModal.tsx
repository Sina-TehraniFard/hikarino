        ...
        <WarmBox className="flex flex-col overflow-hidden">
          ...
          <div className="px-6 md:px-8 pb-8 md:pb-10">
            ...
            <div className="w-full mb-6 mt-4">
              <div
                className={`bg-gradient-to-b from-emerald-100/80 to-emerald-50/60 border border-emerald-300 rounded-lg transition-all duration-500 ease-in-out ${isSecurityExpanded ? "pb-4" : ""}`}
              >
                <button
                ...
                {/* 展開時の詳細情報 */}
                <div
                  className={`transition-all duration-500 ease-in-out ${isSecurityExpanded ? "max-h-48 opacity-100" : "max-h-0 opacity-0"}`}
                  ...
                  <div className="px-4 pb-2">
                    <div className="text-xs text-text-primary leading-relaxed space-y-3">
                      <p className="flex items-start gap-2">
                        ...
                        <span>
                          安心してご購入いただけるように、世界中で利用されている決済サービス「
                      ...
                      <p className="flex items-start gap-2">
                        <span className="text-emerald-600 mt-0.5">●</span>
                        <span>
                          ご入力いただくカード情報は暗号化され、Stripeのセキュアな環境下でのみ安全に処理されます。
                        </span>
                      </p>
                      <p className="flex items-start gap-2">
                        <span className="text-emerald-600 mt-0.5">●</span>
                        <span>
                          ご自身の大切な情報が外部に漏れることはありません。
                        </span>
                      </p>
                      <a
                        href="https://stripe.com/jp/resources/more/secure-payment-systems-explained"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-emerald-700 hover:text-emerald-800 hover:underline inline-flex items-center gap-1 font-medium mt-2"
                      >
            ...
            <div className="w-full flex flex-col gap-4 mb-8">
              {purchaseOptions.map((opt) => (
                <div
                  key={opt.coins}
                  className={opt.recommended ? "relative" : ""}
                >
                  ...
                  >
                    ...
                    <div className="flex flex-col justify-between z-10 flex-1 py-4">
                      <div className="flex flex-col items-start">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-2xl font-bold ${opt.recommended ? "text-amber-800" : "text-text-primary"}`}
                          >
                            {opt.coins.toLocaleString()}
                          </span>
                          <span
                            className={`text-sm font-medium ${opt.recommended ? "text-amber-700" : "text-text-secondary"}`}
                          >
                            コイン
                          </span>
                        </div>
                        ...
                        <span
                          className={`text-xs mt-1 block ${opt.recommended ? "text-amber-700" : "text-text-secondary"}`}
                        >
                          1占い相当:{" "}
                    ...
                    <div className="flex flex-col justify-center items-end z-10 py-4 min-w-[120px]">
                      <div className="flex flex-col items-end">
                        {opt.discount && opt.originalPrice && (
                          <span
                            className={`text-sm line-through font-medium leading-5 h-5 ${opt.recommended ? "text-amber-600" : "text-gray-400"}`}
                          >
                            {opt.originalPrice.toLocaleString()}円
                          </span>
                        )}
                        {!opt.discount && <div className="h-5"></div>}

                        <div className="text-right mt-1">
                          <span
                            className={`text-3xl font-bold leading-9 ${opt.recommended ? "text-red-600" : "text-text-primary"}`}
                          >
                            {opt.priceStr}
                          </span>
                        </div>
            ...
            <button
              onClick={onClose}
              className="w-full text-text-secondary hover:text-text-primary font-medium px-6 py-3 rounded-lg transition-all duration-200 mt-6"
            >
              後で購入する
            </button>
          </div>
