  ...
  return (
    <div className="flex relative overflow-hidden">
      ...
      <div className="flex-1 md:ml-72 overflow-hidden">
        {/* モバイル版（768px未満）- 従来と同じ */}
        <div className="md:hidden w-full max-w-lg mx-auto relative px-6 space-y-6 pb-12">
          <Header
            user={user || { displayName: "ゲスト", email: "", uid: undefined }}
            coins={user ? coins : 0}
            onLogout={handleLogout}
          ...
          {cards.length === 0 && (
            <Button onClick={handleDrawCardsClick} variant="magical" fullWidth>
              タロットを引く
            </Button>
          )}
          ...
          <div className="mt-10 w-full max-w-md">
            ...
            {showWaitingAnimation ? (
              <WaitingAnimation progress={streamingProgress} />
            ) : (
        ...
        {/* PC版（768px以上）- ガラス背景付き */}
        <div className="hidden md:block w-full max-w-lg mx-auto relative p-4">
          <GlassBox className="transition-all duration-500 ease-in-out">
            <div className="px-6 py-6 space-y-6">
              <Header
                user={
                  user || { displayName: "ゲスト", email: "", uid: undefined }
                }
                coins={user ? coins : 0}
                onLogout={handleLogout}
                onRequireLogin={() => setShowLogin(true)}
              ...
              {/* 簡単3ステップ - 不安解消 */}
              <div
                className={`transition-all duration-500 ease-in-out ${isResetting ? "opacity-0 transform -translate-y-4" : "opacity-100 transform translate-y-0"}`}
              >
                <AppIntro onStepClick={handleStepClick} />
              </div>
              ...
              <div
                className={`transition-all duration-500 ease-in-out ${cards.length === 0 && !isResetting ? "opacity-100 transform translate-y-0" : "opacity-0 transform -translate-y-4"}`}
              >
                {cards.length === 0 && (
                  <Button
              ...
              <div
                className={`transition-all duration-700 ease-in-out ${cards.length > 0 && !isResetting ? "opacity-100 transform translate-y-0" : "opacity-0 transform translate-y-4"}`}
              >
                <TarotCards
                  cards={cards}
                  onAllFlipped={() => setAllCardsFlipped(true)}
              ...
              <div
                className={`transition-all duration-500 ease-in-out ${cards.length > 0 && allCardsFlipped && !hasFortuned && !isLoading && !isResetting ? "opacity-100 transform translate-y-0" : "opacity-0 transform -translate-y-4"}`}
              >
                {cards.length > 0 &&
                  allCardsFlipped &&
                  !hasFortuned &&
              ...
              <div className="mt-10 w-full max-w-md">
                ...
                <div
                  className={`transition-all duration-700 ease-in-out ${showWaitingAnimation || hasFortuned || result ? "opacity-100 transform translate-y-0" : "opacity-0 transform translate-y-4"}`}
                >
                  {showWaitingAnimation ? (
                    <WaitingAnimation progress={streamingProgress} />
                  ) : (
        ...
        <MessageDialog
          ...
          type="error"
          message={
            error || "占い中にエラーが発生しました。もう一度お試しください。"
          }
        />
      </div>
    </div>
