...
export const useFortuneFilters = (): FortuneFilters => {
  const filterByText = useMemo(
    () =>
      (fortune: FortuneHistory, query: string): boolean => {
        if (!query) return true;
        const lowerQuery = query.toLowerCase();
        return (
          fortune.question.toLowerCase().includes(lowerQuery) ||
  ...
  const filterByCard = useMemo(
    () =>
      (fortune: FortuneHistory, cardName: string): boolean => {
