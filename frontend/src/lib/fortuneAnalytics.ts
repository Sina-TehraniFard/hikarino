...
export function analyzeFortuneHistory(
  ...
  // 特別な洞察
  const specialInsight = generateSpecialInsight(
    fortunes,
    mostFrequentCard,
    dominantElement[0]
  );
...
function analyzeReadingFrequency(fortunes: FortuneHistory[]): {
  average: string;
  pattern: string;
} {
  ...
  const sortedFortunes = [...fortunes].sort(
    (a, b) => (a.timestamp?.seconds || 0) - (b.timestamp?.seconds || 0)
  );

  const firstDate = new Date(
    (sortedFortunes[0].timestamp?.seconds || 0) * 1000
  );
  const lastDate = new Date(
    (sortedFortunes[sortedFortunes.length - 1].timestamp?.seconds || 0) * 1000
  );
