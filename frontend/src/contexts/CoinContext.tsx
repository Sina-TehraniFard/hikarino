...
const CoinContext = createContext<
  CoinContextType & {
    onCoinPurchase: (
      callback: (oldCoins: number, newCoins: number) => void
    ) => (() => void) | undefined;
  }
>({
  coins: 0,
...
export const CoinProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [coinPurchaseCallback, setCoinPurchaseCallback] = useState<
    ((oldCoins: number, newCoins: number) => void) | null
  >(null);

  const fetchCoins = useCallback(
    async (triggerAnimation = false) => {
      if (!user?.uid) {
        setCoins(0);
        return;
      }
  ...
  const consumeCoins = useCallback(
    async (amount: number) => {
      if (!user?.uid) return;
  ...
  const registerPurchaseCallback = useCallback(
    (callback: (oldCoins: number, newCoins: number) => void) => {
  ...
  return (
    <CoinContext.Provider
      value={{
        ...
        consumeCoins,
        onCoinPurchase: registerPurchaseCallback,
      }}
    >
      {children}
    </CoinContext.Provider>
