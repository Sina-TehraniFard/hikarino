...
export const spendCoin = onCall(async (request: CallableRequest) => {
  ...
  // 使いたいコインの枚数（デフォルト1）
  const amount =
    typeof request.data?.amount === "number" && request.data.amount > 0
      ? request.data.amount
      : 1;
...
export const addCoin = functions.https.onRequest(async (req, res) => {
  ...
  return corsHandler(req, res, async () => {
    ...
    console.log(
      "metadata.coinAmount:",
      req.body.data?.object?.metadata?.coinAmount
    );
    ...
    console.log(
      `ユーザーID：${uid} に${coinAmount}枚のコインを付与（FieldValue.increment 使用）`
    );
